      ...
      , function(result){  
        if (result.status == "success") {
          ...
          if(result.storage == null & color == gray){
            console.log("성공22222?????" + saveList[index].price_min);
	...
	pingSearch : function(api, pgno, callback, result, sort) {
		$.ajax({
			...
			success : function(responseData, textStatus, errorThrown) {
				if (saveList == null) {
					// Object deep copy 사용
					saveList = $.extend(true, {}, responseData.channel.item);
				...
				// saveList responseData transformData 가 동시에 변경????
				/*************************************************************************/
				console.log("날자변환 이전---------------");
				console.log("responseData : " + responseData.channel.item[3].publish_date);
				console.log("saveList : " + saveList[3].publish_date);
								
				// Object deep copy 사용
				var transformData = $.extend(true, {}, responseData);
				yyyyMMddpriceResultFormat(transformData)			
				
				//var transformData = $.extend({}, yyyyMMddpriceResultFormat(responseData));
				//var transformData = yyyyMMddpriceResultFormat(responseData);
				
				console.log("이후---------------");
				console.log("responseData : " + responseData.channel.item[3].publish_date);
				console.log("saveList : " + saveList[3].publish_date);
				console.log("transformData : " + transformData.channel.item[3].publish_date);
				
				/**************************************************************************/

				console.log("가격 포맷 이전---------------");
				console.log("responseData : " + responseData.channel.item[3].price_min);
				console.log("saveList : " + saveList[3].price_min);
				console.log("transformData : " + transformData.channel.item[3].price_min);				
...
function priceFormat(transformData) {
	...
	for ( var i in transformData.channel.item) {
		...
		transformData.channel.item[i].price_min = price.split(/(?=(?:\d{3})+(?:\.|$))/g)
				.join(',');
	}
